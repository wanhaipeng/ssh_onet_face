...
def forward_net(net, blob, im_scale, pyramid='False'):
    ...
    print ('im_info: ', blob['im_info'].shape)
    ...
    print ('inference output blobs: ', len(blobs_out))
    print ('m3 shape: ', net.blobs['m3@ssh_bbox_pred_output'].data.shape, net.blobs['m3@ssh_cls_prob_reshape_output'].data.shape)
    print ('m2 shape: ', net.blobs['m2@ssh_bbox_pred_output'].data.shape, net.blobs['m2@ssh_cls_prob_reshape_output'].data.shape)
    print ('m1 shape: ', net.blobs['m1@ssh_bbox_pred_output'].data.shape, net.blobs['m1@ssh_cls_prob_reshape_output'].data.shape)
    ...
    if pyramid:
        ...
        for i in range(1,4):
            ...
            print ('output boxes shape: ', cur_boxes.shape)
...
def onet_forward(o_net, cls_dets, im, thresh=0.7):
    ...
    roi = out['conv6-2']
    pts = out['conv6-3']
    rectangles = tools.filter_face_48net(cls_prob, roi, pts, cls_dets, im.shape[1], im.shape[0], thresh) 
    return rectangles



def detect(net, o_net, im_path, thresh=0.05, visualize=False, timers=None, pyramid=False, visualization_folder=None):
    """
    ...
    if not pyramid:
        ...
        print ('pyramid_scales: ', pyramid_scales)
        for blob in im_blobs:
          print ('im_blob_shape: ', blob['data'].shape)
    ...
    cls_dets = dets[keep, :]
    landmark_dets = onet_forward(o_net, cls_dets, im, 0.7)
    print ('landmark_dets type: ', type(landmark_dets), len(landmark_dets))
    if visualize:
        plt_name = os.path.splitext(imfname)[0] + '_detections_{}'.format(net.name)
        visusalize_detections(im, landmark_dets, plt_name=plt_name, visualization_folder=visualization_folder)
    timers['misc'].toc()
